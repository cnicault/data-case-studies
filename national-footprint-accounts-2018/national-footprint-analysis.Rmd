...
I will use the data to analyse the evolution over the last 50 years of the biocapacity and the ecological footprint, for the world, at a region level, and look in some countries with the higher changing rate.
I will then analyse the data for the year 2014 to draw a picture of the situation for that particular year (last year with the data provided), with the main contributors or debtors, and make maps for the biocapacity and footprint per capita, overall, and with contributors and debtors.
...
#path <- file.path(here::here("analysis", "NFA 2018.csv"))
#path <- file.path(here::here("NFA 2018.csv"))
nfa <- read_csv(path)
...
We are missing the details on a large period of time and for many countries, and among big countries that would have an impact (USSR).
Therefore, it won't be accurate to analyse the history for the type of land.

## Time range

This sort of graph can be interesting as well for the rest of the data. Even if there are no missing values for other country, this graph can help visualising if these countries have data for the full period(1961 - 2014). We select only the countries who don't have data starting from 1961.

```{r}
nfa_no_data <-nfa %>% 
  filter(!is.na(crop_land)) %>%
  group_by(UN_region, country) %>%
  summarize(begin = min(year),
            end = max(year))
...
Many countries have a long period without data at all. It is particularly true for Africa, Asia and Europe. Looking at the graph, we can guess that an analysis of the history won't be accurate, so we can visualise the trend, keeping in mind that the real numbers are not probably impacted by the lack of data.
But maybe we need to check if it can't be explained by the geopolitics. For example, we can guess that for Europe, USSR has split in the 90s in Russia, Ukraine, etc...
...
The graph tells us that Yugoslavia and USSR stopped in 1991 and all the countries which start in 1991 come from the separation of these two countries. So for Europe, an analysis from 1961 to 2014 would be complete, except for few years for Ireland, and a gap of a year between the end of Czechoslovakia and the data for Slovaka and Czech Republic.
...
For Africa, Ethiopia PDR is split in Ethiopia and Eritrea, and Sudan (former) split between Sudan and South Sudan.
...
The few missing years for these country which are not consumming a lot won't impact much the overall analysis for the continent accross the 53 years.
...
Part of the missing data are due to countries coming from USSR, so there impact must have been accounted in USSR for the missing period.
The data for the countries in the Persian Gulf are missing until the begining of the 80s, and the data for Saudi Arabia are completely missing except for 2014.
...
To get a picture of the present situation, we will focus on the last year of data, but we need to see if some countries have a end date before 2014.
...
All the countries listed are countries that have splited into other countries, as the previous graph showed, except for New Zealand.
Surprisingly, the data are missing for year 2014, the last year with data provided is 2013. We have two choices to deal with that, either we build a model to predict the values for the year 2014, or we use the number of 2013 for the year 2014, assuming that they would be close.
...
We can see that there is a change around 2000 which a big decrease of biocapacity since, and a relative stability for the total consumption. We could build up the missing data by prolonging the decrease linearly for one more year for the biocapacity and use the mean of the last 13 years for the total consumption. But there are 8 variables to build up, and the goal of the analysis is not a focus on New Zealand, I choose the lazy way and assume that the inaccuracy for that year for New Zealand won't change the global analysis for the world or even for Oceania.

For the analysis of the current situation I will therefore use the year 2014 for every country and report the number of 2013 for New Zealand.
...
```{r}
path <- file.path("~","Perso","R","analysis", "API_NY.GDP.PCAP.KD_DS2_fr_csv_v2_11733.csv")
...
Lets see if there are differencies between the GDP in our data and the GDP in the world bank data. We will limit the 
request to differencies above 5 %
...
Let see which missing values can be found in these data.
...
The missing values for theses countries can be explain by the political situation (Eritra, North Korea), the War (Syria, Somalia).
French Polynesia, Guadeloupe, Martinique & Réunin are french territories, and we can guess that there data are part of the french GDP. 

We will continue the analysis with theses missing values.
...
The total consomption is growing 5.2 faster than the biocapacity.
...
The Biocapacity per capita has dramatically reduce, even though as seen on the previous graph, the total biocapacity increased. This could be imputed to the increase of population.
The consumption per capita is above the consumption per capita since 1971.
...
The population has been multiplied by `r pop_rate$prop` from 1960 to 2014, from 3.07 billions to 7.26 billions. Each year the population increase by `r pop_rate$rate` people.

It seems that there is a correlation between the increase of the population and the drop of the biocapacity per capita, and between the total consumption and the population. Let's find out how strong this correlation is. We can guess that it will be strong because the BiocapTotGHA and the population are both linear, and the BiocapPerCap is define by these 2 variables.
...
# Tester la normalité des distribution pour savoir si utiliser pearson ou spearman
...
Not suprising, We can confirm that the biocapacity per capita is higly negatively correlated with the population (-0.9994) and the test is higly sifnigicative p-value < 0.001.
The total consumption is higly positively correlated to the population (0.9967), and the test is higly sifnigicative p-value < 0.001.
...
At the world scale, the commerciales exchanges balanced out, so the export and import follows the same patterns, and the consumption and production too. The purpose of the visualisation is to show the global increase in all area, production/consumption, import/export. The total biocapacity increases a bit too, but the biocapacity per capita has been divided by 2 during the period 1961 - 2014. (beware, all the visualisation are on a free scale)
...
The biocapactity per capita decreases in every region of the world. The rate seems particularly high in Oceania and Latin America & Caribbean.
...
Focusing on Oceania, we can see that most of the biocapacity (and the decrease of it) is due to 3 countries, Australie, New Zealand, and at a lower level Papua New Guinea. Australia is a huge country with a low density of population.
...
In the Latin America, the higher biocapacity and the biggest decrease is due to the French Guyana. 

The visualisation per region shows that within a region there are big variations, there is a big range of values with some country having a big amount, balanced by many countries with a small amount. Ocenia is the region with the highest biocapacity per capita, but looking in detail, some country in other region have far more biocapacity per capita, but the overall amount is reduce due to the number of countries in that region with a very low value.
...
It's probably more interesting to look at the top countries for biocapacity and consumption, rather than by region which are composed of disparate countries.

We can however make a visualisation to see at a region level the evolution between biocapacity and consumption, and which region have a higher consumption, and the trend over time
...
We can see that Asia, Europe and North America have a consumption above biocapacity during all the period shown. Africa's consumption has just recently been exceed the biocapacity. The other region still have a biocapacity above consumption. Even if the biocapacity increases in Asia, the rate at which the consumption grows and the growth of the gap between biocapacity and consumption is impressive compare to the other regions.
...
Let's have a look at the top and bottom countries for the biocapacity rate
...
The highest lost in biocapacity per capita is French Guiana. Is it due to a decrease of the total biocapacity, the increase of the population or both ?
...
The division by 2 of the biocapacity of French Guyana is mainly due to the increase of the population which has almost being multiply by 2. During that time, the total biocapacity has slightly decreased (3%).
...
The first countries with the highest consumption per capita are wealthy countries with low population compared to their life level (Qatar, Luxembourg). We find the countries from middle east producing oil (Qatar, Emirates, Bahrain, Kuwait, Oman, Saudi Arabia). Countries with a high consumption (China, India) but with a high population don't have a high consumption per capita, where countries with a lower number of inhabitants (USA, Canada) are in the firsts place for consumption per capita. Surprisingly Mongolia is fourth, and Trinidad and Tobago fourteenth, which are country I wasn't expecting in the top 15.
...
Looking at the ecological debt (consumption - biocapacity), the highest are the wealthy countries with a small or desertic territory. The first countries are still the oil producer from middle east, luxembourg, which has a small territory and therefore poor biocapacity.
...
plt1 <- nfa2014 %>%
  ...
  scale_y_log10()+
  scale_x_log10() +
  labs(x = "gdp per capita (USD 2010)",
     y = "Consumption per Capita (GHA/person)",
     title = "Ratio between consumption per capita and GDP per capita",
     subtitle = "footprint = Consumption - biocapactiy")
...
On a log scale, there's a strong correlation between the consumption per cap and the gdp per cap. We can see that the countries in Asia spread along the graph, with Timor at the lowest consumption and Qatar with the highest. Europe and North America are at the top half of the graph, where Africa is at the bottom half. Latin America and the Caribbean are located in the middle. Anyway, the region overlap, and therefore, Countries from different region can be close, the top of african countries are closed to the bottom of european and the middle of Latin american (Romania, Gabon, Mexico for example)
...
The total biocapacity compared to the surface of the territory is clearly seen on the map.
...
nfa2014 %>%
  ...
  labs(x = "Countries",
       ...
       title = "15 first countries with the consumption")  

```

The top list for consumption is not surprising, China and India, mainly due to the large population and the growth of the life still. USA, Russia and Brazil complete the top 5. We can find the top countries for economical activities, such as Japan, Germany, France, UK ..
...
Let's have a look hot they are balanced for theses countries.
...
There is a strong positive correlation between the carbon emission and the consumption.
...
The first higher emitters of CO2 are the the same countries that are the first in consumption. Some countries changed position (Brazil, Iran).
...
nfa2014 %>%
  ...
  labs(x = "Total Biocapacity excedent (GHA)",
       ...
       subtitle = "contribution = biocapactiy - Consumption")    

```

I define the contribution as the difference between the biocapacity and the consumption, to see which countries have still a positive impact. Brazil is far ahead, and we can find Canada, Russia, Australia, which even if they are in the top countries for consumption have still a positive impact due to their high biocapacity. Mainy countries are from latin America and Africa, which can make sense due to their natural environnement and low consumption.
...
I define the debt as the difference between the consumption and the biocapacity, to see which countries have still a negative impact. The top 3 countries are China, USA and India. We can find as well the european countries with the most active economy (Germany, UK, Italy)
...
The map display the contribution for each countries as a percentage of their biocapacity and the debt as a percentage of their consumption.
In percentage, the impact is balanced, and the main countries (China, USA) have a lower impact than other like Iran. It can be seen as how vertuous or not a country is.
But what is interested at a world level is the global impact on the plant. Let's see the impact in total with the next map.
...
Looking at the map with the total contribution or debt, we can see that China, USA and India are really highlighted. The countries with a positive impact brazil, Canada, Russia, can't counterbalance the level of the countries with the highest negative impact.


# Countries who consummes half of the total consumption

Looking at the previous map, it seems that only few countries have a huge impact on the total consumption on the planet.
...
Only 12 countries consumes 2/3 of the total world consumption.
China, USA, India, Russia and Brazil consumes more thant half of the total world consumption.
...
nfa_country_lvl %>%
  ...
  labs(x = "Year",
       ...
       title = "Difference between Biocapacity and EFConsTotGHA for the first 5 biggest consummers",
